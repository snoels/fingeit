...
import torch
import pandas as pd
from scripts.evaluation.file_access import FileAccess
...
class BaseEvaluator:
    ...
    def __init__(self, model_path:str, remove_temp_file:bool|None=None, temp_storage_file:str|None=None) -> None:
    ...
    def _get_response(self, response) -> str:
        lines = [self.tokenizer.decode(g, skip_special_tokens=True, clean_up_tokenization_spaces=False) for g in response]
    ...
    def _to_dataloader(self, dataset:Dataset) -> DataLoader:
        def collate_fn(batch):
            inputs = self.tokenizer(
                [f["text"] for f in batch], return_tensors='pt',
                padding=True,
    ...
    def _get_predictions(self, dataset:Dataset) -> Dataset:
        model = AutoModelForCausalLM.from_pretrained(self.model_path,  device_map="cuda") #do not move this to the constructor, out of memory
        for idx, inputs in enumerate(tqdm(self._to_dataloader(dataset))):
            inputs = {key: value.to(model.device) for key, value in inputs.items()}
            res = model.generate(**inputs, max_length=2048, eos_token_id=self.tokenizer.eos_token_id)
    ...
    def _add_predictions(self, dataset:Dataset) -> Dataset:
        try:
            ...
            print(f"woepsie, somehting happend during prediction time. You can view already processed data in the temp file: {self.file_access.file}", e)
